...
function rotateRadius(event) {
    ...
    if (!radius.initialPosition) {
        radius.initialPosition = {
            x: event.clientX || event.touches[0].clientX,
            y: event.clientY || event.touches[0].clientY
        };
        radius.initialAngle = Math.atan2(event.clientY - centerY, event.clientX - centerX);
    }

    // Calculate current angle based on initial angle and current touch/mouse position
    const initialAngle = radius.initialAngle;
